...
<%
	// 업로드할 디렉토리 위치를 구해옴
	String path = request.getRealPath("upload");	// upload는 name을 가리킨다.
	...
	// jsp:usebean 액션태그를 사용하지 못해서 DTO클래스에 직접 입력한다.
...
//	board.setUpload(upload);	// 첨부파일을 수정하지 않으면 NULL값이 저장된다.
	...
	BoardDataBean old = dao.getContent(num);	// SQL문 실행
	
	if(upload != null){	// 첨부 파일을 수정한 경우
		board.setUpload(upload);
	}else{	// 첨부 파일을 수정 하지 않은 경우
